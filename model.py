...
class WeightedL1(nn.Module):
    ...
    def forward(self, output, target, weight):
        ...
        lossvalue = lossvalue*weight
        return lossvalue.mean()
...
class StdLoss(nn.Module):
    ...
    def forward(self, output, target, weight):
        loss1 = torch.abs(output - target)
        loss1 = loss1*weight
        loss1 = loss1.mean()
        std = output.std()
        loss2 = 0.5/(1+std)
        print(loss1, loss2)
        lossvalue = loss1+loss2
        return lossvalue

class ConvLearner():
    '''Training loop'''
    def __init__(self, epochs=NUM_EPOCHS):
        self.model = JointNetwork()
    ...
    def train(self, train_loader, model, criterion, optimizer, epoch):
        ...
        for _, data in enumerate(train_loader):
            seq, seq2, target = data
            target = torch.stack(target)
            # weight = torch.stack(weight)
            output = model(seq, seq2)
            loss = criterion(output, target)
            train_loss += loss.item()
            train_mean += output.mean()
            train_std += output.std()
        ...
        return train_loss, train_mean, train_std
    ...
    def step(self):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            epoch_loss, u, std = self.train(self.train_loader, self.model, self.criterion, self.optimizer, epoch)
            # cross validation
            total_val_loss = 0
            with torch.no_grad():
                for _, data in enumerate(self.cv_loader):
                    ...
                    # weight = torch.stack(weight)
                    self.model.eval()
                    val_outputs = self.model(seq, seq2)
                    v1 = val_outputs
                    val_loss = self.criterion(v1, target)
                    total_val_loss += val_loss.item()
                epoch_val_loss = total_val_loss/len(self.cv_loader)
                if epoch % 1 == 0:
                    self.status(epoch, epoch_loss, u, std, epoch_val_loss, lr)
    ...
    def status(self, epoch, u, std, epoch_loss, epoch_val_loss, lr):
        print('epoch {0}/{1}:\n train_loss: {2} mean: {3} std: {4} val_loss: {5} learning_rate: {6}'
        .format(epoch, self.epochs, u, std, epoch_loss, epoch_val_loss, lr))
